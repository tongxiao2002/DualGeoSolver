...
def insert_sep_into_programs(program: list):
    extended_program = []
    for idx, item in enumerate(program):
        if idx > 0 and item[:2] not in ['V_', 'C_', 'N_']:
            # this item is operator
            extended_program.append('@@sep@@')
        extended_program.append(item)
...
class SeqReader(DatasetReader):
    ...
    def load_metadata(self, filepath: str):
        if "GeoQA-Pro" in filepath:
            sub_dict_path = "GeoQA-Data/GeoQA-Pro/sub_dataset_dict.pkl"
            knowledge_data_map_path = "GeoQA-Data/GeoQA-Pro/geometry-knowledge-data-map.json"
            knowledge_explanation_path = "GeoQA-Data/GeoQA-Pro/geometry-knowledges-chinese.json"
        elif "GeoQA+" in filepath:
            sub_dict_path = "GeoQA-Data/GeoQA+/sub_dataset_dict.pkl"
            knowledge_data_map_path = "GeoQA-Data/GeoQA+/geometry-knowledge-data-map.json"
            knowledge_explanation_path = "GeoQA-Data/GeoQA+/geometry-knowledges-chinese.json"
        else:
            raise ValueError("Must be 'GeoQA-Pro' or 'GeoQA+' datasets.")

        with open(sub_dict_path, 'rb') as file:
            subset_dict = pickle.load(file)
        self.subset_dict = subset_dict

        self.knowledge_data_map = json.load(open(knowledge_data_map_path, "r", encoding="utf-8"))
        self.knowledge_explanations = json.load(open(knowledge_explanation_path, "r", encoding="utf-8"))
        self.knowledge2index = {know: idx for idx, know in enumerate(self.knowledge_explanations.keys())}

    @overrides
    def _read(self, file_path: str):
        self.load_metadata(filepath=file_path)
        ...
        with open(file_path, 'rb') as f:
            ...
            if "train" in file_path:
                ...
                for sample in dataset:
                    ...
                    if data_id not in self.knowledge_data_map:
                        continue
    ...
    def text_to_instance(self, sample) -> Instance:
        ...
        # img_rgb = np.array(img_rgb).transpose((2, 0, 1)) / 255.
        ...
        ans = insert_sep_into_programs(ans)
        ...
        type = self.subset_dict[str(sample['id'])]
        ...
        # make sure that sum of the step_knowledges wouldn't be larger than all knowledges
        if str(data_id) in self.knowledge_data_map:
            step_knowledges = self.knowledge_data_map[str(data_id)]
            knowledges = set([item for step_knows in step_knowledges for item in step_knows])
            if len(knowledges - set(exam_points)) != 0:
                print(data_id, end=' ')

            knowledge_labels = np.zeros((len(step_knowledges), len(self.knowledge2index)), dtype=np.float32)
            for idx, step_knows in enumerate(step_knowledges):
        ...
        fields['knowledge_labels'] = ArrayField(knowledge_labels)
