...
class Encoder(Model):
    ...
    def forward(self, src, source_mask, merge_mask, merge_cap_mask, img_feats, img_mask):
        ...
        # if merge_mask is not None and merge_cap_mask is not None:
...
class SimpleSeq2Seq(Model):
    ...
    def forward(self,  # type: ignore
        ...
        with torch.no_grad():
            ...
            # img_feats_ = img_feats_.view(*img_feats_.shape[:2], -1).transpose(1, 2)
    ...
    def _init_decoder_state(self, state, lang_feats, img_feats, img_mask):
        ...
        inference_hidden = self.inference_transform(feat)
        state["concat_feature"] = decoder_hidden

        state["decoder_hidden"] = decoder_hidden
        state["decoder_hidden_states"] = decoder_hidden.unsqueeze(1)
        state["guiding_hidden_state"] = inference_hidden
        state["guiding_context"] = torch.zeros_like(inference_hidden)
    ...
    def _prepare_output_projections(self,
        ...
        if torch.any(update_knowledges).item():
            ...
            logits_for_selecting = curr_labels if self.training else curr_logits
            threshold = 0.6 if self.training else self.knowledge_threshold
            ...
            # score from logits, every token in a knowledge explanation will have the same score
            ...
            knowledge_indices += update_knowledges.long()
