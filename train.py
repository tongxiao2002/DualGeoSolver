...
from beam_search import FixedBeamSearch
from knowledge_utils import select_explanations_by_prediction
from inference_net import GoalGenerationModule, KnowledgeInjectionModule
...
from transformers import AutoModel, BertTokenizer, ViTMAEForPreTraining, ViTFeatureExtractor
...
class Encoder(Model):
    ...
    def forward(self, src, source_mask, merge_mask, merge_cap_mask, img_feats, img_mask):
        ...
        # if merge_cap_mask is not None and merge_mask is not None:
        ...
        #     att_embed = self.merge_att(lstm_embedding, img_feats, merge_cap_att_mask, img_mask)

        #     att_embedding = (att_embed * merge_cap_att_mask_.unsqueeze(-1)) + (lstm_embedding * merge_cap_att_mask.squeeze(1).squeeze(1).unsqueeze(-1))
...
class SimpleSeq2Seq(Model):
    ...
    def __init__(self,
                 ...
                 knowledge_explanation_file: str,
        ...
        self.knowledge_injection_module = KnowledgeInjectionModule(
            ...
            inference_hidden_dim=512,
            decoder_hidden_dim=512,
            sep_token_id=self.sep_token_id,
        )
        self.goal_generation_module = GoalGenerationModule(
            num_layers=2,
            d_model=512,
            nhead=8,
            feedforward_dim=2048,
        )
        # self.knowledge_head = FeedForwardNet(512, 512, 50)
        ...
        knowledge_explanation = [desc for desc in self.knowledge_explanations.values()]
        knowledge_tokenized = self.bert_tokenizer(
        ...
        self.knowledge_explanation_input_ids = knowledge_tokenized['input_ids'].cuda()
        self.knowledge_explanation_attention_mask = knowledge_tokenized['attention_mask'].cuda()
        ...
        self._beam_search = FixedBeamSearch(self._end_index, max_steps=max_decoding_steps, beam_size=beam_size)
        ...
        if self._attention:
            ...
            self._decoder_input_dim = self._decoder_output_dim + target_embedding_dim + self.inference_hidden_dim

        else:
            # Otherwise, the input to the decoder is just the previous target embedding.
            self._decoder_input_dim = target_embedding_dim + self.inference_hidden_dim
        ...
        # self.load_state_dict(torch.load("runs/DPE-NGS-with-label-augmentation/best.th", map_location="cuda:0"))
    ...
    def forward(self,  # type: ignore
        ...
        with torch.no_grad():
            ...
            # img_feats_ = self.resnet(image)
            # img_feats_ = img_feats_.reshape(img_feats_.shape[0], img_feats_.shape[1], -1).transpose(1, 2)

        img_feats_ = self.channel_transform(img_feats_) #(64+1, 768)
        # img_feats = img_feats_
        ...
        state["vision_hidden_states"] = img_feats
        state["vis_attention_mask"] = (~img_mask.squeeze()).long()
        state["knowledge_labels"] = knowledge_labels
            ...
            if target_tokens and self._bleu:
                ...
                for b in range(bs):
                    ...
                    for i in range(self._beam_search.beam_size):
                        ...
                        hypo = [item for item in hypo if item != "@@sep@@"]
                        # print(hypo)
    ...
    def _init_decoder_state(self, state, lang_feats, img_feats, img_mask):
        ...
        decoder_hidden = self.decode_transform(feat)
        guiding_hidden = self.inference_transform(feat)
        state["concat_feature"] = decoder_hidden

        state["decoder_hidden"] = decoder_hidden
        state["decoder_hidden_states"] = decoder_hidden.unsqueeze(1)
        state["guiding_hidden_state"] = guiding_hidden
        state["guiding_context"] = torch.zeros_like(guiding_hidden)
        # C0 shape: (batch_size, decoder_output_dim)
        state["decoder_context"] = torch.zeros(batch_size, self._decoder_output_dim).cuda()
        knowledge_indices = torch.zeros(batch_size, dtype=torch.long, device=decoder_hidden.device)
        state["knowledge_indices"] = knowledge_indices
    ...
    def _forward_loop(self,
        ...
        for timestep in range(num_decoding_steps):
            ...
            # recurrent decoding
            output_projections, state = self._prepare_output_projections(input_choices, state, False)
        ...
        if target_tokens:
            ...
            if "knowledge_loss" in state:
                output_dict["loss"] += state["knowledge_loss"]
    ...
    def compute_multilabel_loss(self, logits, knowledge_labels):
        loss_fn = nn.BCEWithLogitsLoss()
        loss = loss_fn(logits, knowledge_labels)
        return loss

    def _prepare_output_projections(self,
                                    last_predictions: torch.Tensor,
                                    state: Dict[str, torch.Tensor],
                                    use_gt_descriptions: bool = False) -> Tuple[
        ...
        update_knowledges = torch.logical_or(
            input=last_predictions == self.sep_token_id,
            other=torch.tensor(state['decoder_hidden_states'].shape[1] == 1, device=last_predictions.device),
        )

        # get guiding hidden state
        guiding_hidden_state = state["guiding_hidden_state"]
        guiding_context = state["guiding_context"]
        if torch.any(update_knowledges).item():
            knowledge_indices = state["knowledge_indices"]
            knowledge_labels = state["knowledge_labels"]
            num_labels = knowledge_labels.shape[-1]
            curr_labels = torch.gather(
                input=knowledge_labels[update_knowledges],
                dim=1,
                index=knowledge_indices[update_knowledges].view(-1, 1, 1).repeat(1, 1, num_labels),
            ).squeeze(1)
            goal_hidden_state = self.goal_generation_module(
                decoder_hidden_states=state["decoder_hidden_states"][update_knowledges],
                encoder_hidden_states=encoder_outputs[update_knowledges],
                encoder_attention_mask=source_mask[update_knowledges],
            )[:, -1, :]
            curr_logits = self.knowledge_head(goal_hidden_state)
            curr_loss = self.compute_multilabel_loss(
                logits=curr_logits,
                knowledge_labels=curr_labels,
            )

            if self.training:
                if not use_gt_descriptions:
                    logits_for_selecting = curr_logits
                    threshold = self.knowledge_threshold
                else:
                    logits_for_selecting = curr_labels
                    threshold = 0.6
            else:
                logits_for_selecting = curr_logits
                threshold = self.knowledge_threshold

            curr_knowledge_input_ids, curr_knowledge_attention_mask, curr_indices = select_explanations_by_prediction(
                logits=logits_for_selecting,
                knowledge_explanation_input_ids=self.knowledge_explanation_input_ids,
                knowledge_explanation_attention_mask=self.knowledge_explanation_attention_mask,
                max_length=self.knowledge_max_length,
                threshold=threshold,
            )
            curr_knowledge_hidden_states = self._encoder(
                src=curr_knowledge_input_ids,
                source_mask=curr_knowledge_attention_mask,
                img_feats=None,
                img_mask=None,
                merge_mask=None,
                merge_cap_mask=None,
            )
            # score from logits, every token in a knowledge description will have the same score
            # [torch.sum(update_knowledges), seq_len]
            # if self.training and not use_gt_descriptions:
            #     score_per_token = torch.gather(
            #         input=torch.sigmoid(curr_logits),
            #         dim=1,
            #         index=curr_indices,
            #     )
            #     # multiply score to hidden states
            #     curr_knowledge_hidden_states = torch.multiply(
            #         input=curr_knowledge_hidden_states,
            #         other=score_per_token.unsqueeze(-1),
            #     )

            guiding_hidden_state_updated = guiding_hidden_state[update_knowledges]
            guiding_context_updated = guiding_context[update_knowledges]

            guiding_hidden_state_updated, guiding_context_updated = self.knowledge_injection_module.forward_step(
                decoder_hidden_state=decoder_hidden[update_knowledges],
                goal_hidden_state=goal_hidden_state,
                encoder_hidden_states=curr_knowledge_hidden_states,
                encoder_attention_mask=curr_knowledge_attention_mask,
                vision_hidden_states=state["vision_hidden_states"][update_knowledges],
                last_guiding_hidden_state=guiding_hidden_state_updated,
                last_guiding_context=guiding_context_updated,
            )
            guiding_hidden_state[update_knowledges] = guiding_hidden_state_updated
            guiding_context[update_knowledges] = guiding_context_updated

            knowledge_indices = knowledge_indices.clone() + update_knowledges.long()
            knowledge_indices = torch.clamp_max(
                knowledge_indices,
                max=knowledge_labels.shape[1] - 1,
            )
            state["guiding_hidden_state"] = guiding_hidden_state
            state["guiding_context"] = guiding_context
            state["knowledge_indices"] = knowledge_indices

            if self.training:
                if "knowledge_loss" not in state:
                    state["knowledge_loss"] = curr_loss
                else:
                    state["knowledge_loss"] += curr_loss
        ...
        if self._attention:
            ...
            decoder_input = torch.cat((guiding_hidden_state, attended_input, embedded_input), -1)

        else:
            # shape: (group_size, target_embedding_dim)
            decoder_input = torch.cat((guiding_hidden_state, embedded_input), dim=-1)
        ...
        state["decoder_hidden_states"] = torch.cat([state["decoder_hidden_states"], decoder_hidden.unsqueeze(1)], dim=1)
